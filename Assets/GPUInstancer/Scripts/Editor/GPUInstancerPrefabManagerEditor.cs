...
namespace GPUInstancer
{
    ...
    public class GPUInstancerPrefabManagerEditor : GPUInstancerManagerEditor
    {
        ...
        public override void DrawRegisteredPrefabsBoxList()
        {
            ...
            else if (Application.isPlaying && _prefabManager.runtimeDataList != null && _prefabManager.runtimeDataList.Count > 0)
            {
                foreach (GPUInstancerRuntimeData runtimeData in _prefabManager.runtimeDataList)
                {
                    GUILayout.Label(runtimeData.prototype.ToString() + " Instance Count: " +
                        runtimeData.instanceCount, GPUInstancerEditorConstants.Styles.label);
                }
            }
            else
